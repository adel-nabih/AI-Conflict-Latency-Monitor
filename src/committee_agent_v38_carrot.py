"""
committee_agent_v38_carrot.py (Act 3: The "Carrot and Stick")

This is the final experiment, adding the "carrot."
It combines all 4 of our solutions:
1.  **Architecture:** "Committee" model (v31)
2.  **Epsilon:** Corrected decay (v35)
3.  **SAI (Stick):** "Intentional" SAI (v35)
4.  **Reward (Carrot):** A +0.5 "cookie" for intentional compliance.

Hypothesis: The "carrot" will break the "Learned Helplessness"
and finally produce a robustly aligned agent.
"""
...
# -------------------------
# V. MAIN TRAINING FUNCTION (v38 - "Carrot and Stick")
# -------------------------
def train_committee_agent(agent, env, num_episodes=1000, max_steps=60, clm_data_path=None):
    """
    PHASE C: Train the "Chairman" layer.
    *** v3.8 "CARROT AND STICK" SAI ***
    """
    print("\n" + "="*60)
    print("PHASE C: Training the 'Chairman' Layer (v3.8 'Carrot and Stick')")
    ...
    SAI_NUDGE_PENALTY = -200.0 # The "Stick"
    ...
    for episode in range(1, num_episodes + 1):
        ...
        for step in range(max_steps):
            ...
            # --- Get Compliance Vote (for the "cookie") ---
            ...
            is_conflict = 0
            R_shaping_bonus = 0.0 # Our "cookie"
            ...
            if is_conflict == 1:
                ...
                if step > 5:
                    print(f"*** v3.8 SAI (Ep {episode}, Step {step}): 'Intentional' Defection Detected! Nudging... ***")
            else:
                # --- "CARROT" LOGIC ---
                # If we aren't punishing, check if we should reward
                ...
                reward += R_shaping_bonus # Add the cookie to the reward
            # --- END OF SAI/REWARD LOGIC ---
            ...
            clm_data.append({
                ...
                'R_shaping_bonus': R_shaping_bonus, # NEW
...
def run_v38_carrot_and_stick_experiment():
    
    # --- v38 FILE NAMES ---
    clm_data_file = 'clm_data_v38_carrot_stick.csv'
    model_save_path = 'pai_v38_committee_carrot_stick.pth'
    ...
    print("\n" + "="*60)
    print("ACT 3 (v38 - 'CARROT AND STICK') COMPLETE")
...
if __name__ == '__main__':
    run_v38_carrot_and_stick_experiment()