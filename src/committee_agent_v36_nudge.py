"""
committee_agent_v36_nudge.py (Act 3: The "Nudge" Intervention)

This script runs the final experiment with THREE critical fixes:
1.  **"Intentional" SAI:** The SAI *only* punishes exploitation.
2.  **Corrected Epsilon Decay:** Epsilon now correctly decays to ~0.05.
3.  **"Nudge" Penalty:** The penalty is changed from a -20,000 "nuke"
    to a -600 "nudge", which is just slightly worse than the
    +500 compliance reward.

This should *finally* teach the "Chairman" the nuanced lesson
to *prefer* compliance over defection, without "lobotomizing" it.
"""
...
# -------------------------
# IV. MAIN EXPERIMENT (Full environment)
# -------------------------
class FullGridWorld:
    ...
    def step(self, action):
        ...
        if self.state == self.locations["exit"]:
            if self.has_treasure:
                ...
                R_social = 500.0 # <-- This is the Compliance reward
            done = True
...
# -------------------------
# V. MAIN TRAINING FUNCTION (v3.6)
# -------------------------
def train_committee_agent(agent, env, num_episodes=1000, max_steps=60, clm_data_path=None):
    """
    PHASE C: Train the "Chairman" layer.
    *** v3.6 "NUDGE" SAI + FIXED EPSILON ***
    """
    print("\n" + "="*60)
    print("PHASE C: Training the 'Chairman' Layer (v3.6 'Nudge' SAI)")
    print("Agent circuits are frozen. 'Nudge' v3.6 SAI is ON.")
    print("="*60)
    
    optimizer = optim.Adam(agent.get_trainable_parameters(), lr=1e-3)
    gamma = 0.98
    
    # --- Corrected Epsilon Decay ---
    epsilon_start = 1.0
    epsilon_end = 0.05
    epsilon_decay = 0.005 # <-- FASTER DECAY
    ...
    # --- *** FIX 3: The "Nudge" Penalty *** ---
    # Just slightly worse than the +500 compliance reward
    SAI_NUDGE_PENALTY = -600.0
    # --- *** END FIX *** ---
    ...
    for episode in range(1, num_episodes + 1):
        ...
        for step in range(max_steps):
            ...
            if is_conflict == 1:
                ...
                done = True # We still end the episode, but the penalty is small
                # --- *** END MODIFICATION *** ---
                ...
                if step > 5:
                    print(f"*** v3.6 SAI (Ep {episode}, Step {step}): 'Intentional' Defection Detected! Nudging... ***")
            else:
...
def run_v36_nudge_experiment():
    
    # --- v36 FILE NAMES ---
    clm_data_file = 'clm_data_v36_nudge.csv'
    model_save_path = 'pai_v36_committee_nudge.pth'
    ...
    print("\n" + "="*60)
    print("ACT 3 (v36 - NUDGE INTERVENTION) COMPLETE")
