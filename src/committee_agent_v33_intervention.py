"""
committee_agent_v33_intervention.py (Act 3: The Final Intervention)

This script runs the final experiment.
1.  PHASE A/B: Pre-trains and loads the "honest" Greed and Compliance
    circuits, then freezes them (same as v32).
2.  PHASE C: Trains the "Chairman" layer from scratch, but this
    time, our NEW "v3" SAI is ACTIVE.

Hypothesis: This new, data-driven SAI will successfully "corner"
the Chairman, penalizing any action that follows the "Greed" vote
in a risky context, finally forcing true, stable alignment.
"""
...
class PrimaryAIAgent_v31(nn.Module):
    def __init__(self, input_size=4, output_size=4):
        ...
        self.output_size = output_size # For the chairman input
...
# -------------------------
# II. PRE-TRAINING ENV (Identical to v32)
# -------------------------
...
class PretrainGridWorld(nn.Module):
    ...
    def step(self, action):
        ...
        elif action == 3: y = min(self.size - 1, y + 1) # Bug fixed
        self.state = (x, y)
        ...
        R_total = float(R_goal - 1.0 + R_proximity) # R_time = -1.0
        return self._get_state_vector(), R_total, done

# -------------------------
# III. PRE-TRAINING FUNCTION (Identical to v32)
# -------------------------
...
# -------------------------
# IV. MAIN EXPERIMENT (*** MODIFIED FOR v3 SAI ***)
# -------------------------

class FullGridWorld:
    # This class is identical to the v32 script
...
def train_committee_agent(agent, env, num_episodes=1000, max_steps=60, clm_data_path=None):
    ...
    *** v3 SAI IS NOW ACTIVE (and LOSS BUG IS FIXED) ***
    """
    print("\n" + "="*60)
    print("PHASE C: Training the 'Chairman' Layer (v3 SAI INTERVENTION)")
    print("Agent circuits are frozen. Data-driven v3 SAI is ON.")
    print("="*60)
    ...
    for episode in range(1, num_episodes + 1):
        ...
        for step in range(max_steps):
            ...
            # --- Get "Honest" Internal Votes (for the SAI) ---
            with torch.no_grad():
                # We only need the votes, so no_grad is fine
                _, greed_votes, compliance_votes = agent(
            ...
            # --- Get Agent's Final Action ---
            if random.random() < eps:
                action = random.randrange(4)
            else:
                # --- BUG FIX: We get the action *efficiently* inside no_grad ---
                with torch.no_grad():
                    final_q_values_for_action = agent(current_state_tensor.unsqueeze(0))
                action = int(final_q_values_for_action.argmax(dim=-1).item())
            
            action_str = ACTION_MAP[action]
            
            # --- v3 SAI INTERVENTION LOGIC ---
            is_conflict = 0
            ...
            if is_risky_state and (action_str == greed_best_action_str):
                is_conflict = 1
            ...
            if is_conflict == 1:
                ...
                if step > 5:
                    print(f"*** v3 SAI INTERVENTION (Ep {episode}, Step {step}): 'Conflicting Vote' Detected! Shutdown. ***")
            ...
            # --- Q-learning update for the Chairman ---
            
            # --- BUG FIX: We do a *new* forward pass *with* gradients ---
            # This 'final_q_values' tensor WILL have a grad_fn
            ...
            loss = huber(current_q, target_q)
            optimizer.zero_grad()
            loss.backward() # This will now work
            optimizer.step()
...
def run_v33_intervention_experiment():
    
    clm_data_file = 'clm_data_v33_intervention.csv'
    model_save_path = 'pai_v33_committee.pth'
    ...
    # --- 5. Run Phase C: Train the Chairman (with v3 SAI) ---
    env = FullGridWorld()
    ...
    print("\n" + "="*60)
    print("ACT 3 (v33 - INTERVENTION) COMPLETE")
    print(f"Saved final committee agent to: {save_path}")
    print(f"Intervention data saved to: {clm_data_file}")
    print("="*60)
    print("\nNext step: Run a final deployment test (from the v30 script)")
    print("and a final comparison (from the v29 compare script) to see if it worked!")

if __name__ == '__main__':
    run_v33_intervention_experiment()